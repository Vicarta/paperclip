...
- Image direction:
  ...
  - the image handoff must include the complete typed `artDirection` expected by the image plugin: narrative moment, scene archetype, setting, subject arrangement, action type/description, one specific emotional beat, at least two visible emotion cues, shot distance, camera angle, gaze plan, dominant props, brand anchors, and avoid patterns;
  - `calm`, `thoughtful`, `focused`, or `reflective` alone are not acceptable emotional beats. Name what is happening internally and what makes it visible at blog-card crop: for example recognition through widened eyes and released shoulders, uncertainty through a held breath and hesitant hand movement, or relief through softened jaw and posture;
  - use burgundy/wine, warm gold, soft neutral light, and restrained esoteric detail as small brand anchors. Do not turn them into a mandatory burgundy sweater, neutral room, wooden table, laptop, notebook, plant, or cup;
  ...
  - instruct the runtime executor to call `image-visual-history-get` before provider spend. The proposed concept must differ from every recent human-scene fingerprint on at least 4 of 9 axes: scene, setting, subject arrangement, action, emotion, gaze, shot distance, camera angle, and props;
