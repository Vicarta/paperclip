...
- Phase 20 live first pass is applied to prevent repeated writer/validator loops: `SEO Blog Article Writer GPT` exists as a GPT fallback author, validator output must include structured blocker classes, and CMO must route repeated same-class failures to the fallback writer. Phase 24 supersedes the earlier manager heartbeat allowance: routine LLM heartbeats should be disabled for CEO, CMO, and CTO unless a human explicitly approves a temporary exception with expiry.
