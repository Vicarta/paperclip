...
- Phase 22 source implementation is ready for durable production cutover: `notification-contract` supports `delivery_groups`, plugin SDK exposes gated issue attachment reads, and `paperclip-plugin-telegram@0.3.1-paperclip.0` sends grouped issue attachments as Telegram documents with idempotency and audit comments. A live runtime recovery proved the delivery-groups path on [AST-827](/AST/issues/AST-827) with Telegram message id `628`, but the durable follow-up remains a normal app/plugin package cutover instead of relying on persistent plugin-volume alignment.
...
- [ ] **Phase 22: Telegram Attachment Delivery Groups** - Source implementation complete and verified locally. Live runtime proof succeeded for [AST-827](/AST/issues/AST-827), but a durable app/plugin package cutover remains before the temporary direct Bot API delivery path can be considered retired.
...
- [ ] **Image Delivery System Hardening** - Covered by Phase 22 for Telegram package delivery; later image-specific follow-up should focus only on generation/provider quality, not file transport.
...
Next server-side follow-up: finish the durable Phase 22 cutover so ready article/file packages are delivered through the source-controlled Telegram plugin package instead of operator-side Bot API calls or live plugin-volume dependency alignment. Keep `ops/paperclip-production/` updated after every production config change, then design company-scoped plugin settings for shared MCP/provider plugins before moving Semantic Core MCP and other shared tokens into strictly company-local refs. Future upstream release updates must reuse the Phase 16 packaging/compatibility approach, preserve the local-agent API URL invariant, keep human Telegram lifecycle messages owned by one sender path, avoid manual CTO/Observability nudges that duplicate Paperclip recovery actions, and prefer deterministic watchdog checks over frequent no-op LLM heartbeats for expensive article writers.
