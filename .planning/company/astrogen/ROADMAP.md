...
- [ ] **Phase 22: Telegram Attachment Delivery Groups** - Turn issue attachment package delivery into a source-controlled Telegram plugin capability. Keep the existing `attach_files` limit, add grouped delivery packets, use `sendDocument` by default, add idempotency/audit trail, and avoid live `dist` hotfixes that break on future Paperclip updates.
