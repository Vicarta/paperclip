...
- Classify an open article case as productive only when its wrapper has no \`case.fields.blockerClass\`, its stage is not \`external_wait\`, and it exposes either a non-null \`activeWork\` or a positive \`descendantActiveWorkCount\`. Never infer productive WIP from a stage name, a previous CMS draft, a reservation, or a historical linked issue.
- A nonterminal article case with no live work wrapper is \`idle_or_blocked\`, not productive WIP. When it has durable CMS or stage evidence, CMO must keep and repair that same canonical case through its existing owner/automation path; it must not create a replacement article or let the idle case consume another topic slot.
...
- When \`western_astrology_learning\` is deficient, an empty Semantic Core snapshot is not evidence exhaustion. The next continuation must use the approved curriculum graph to create or reuse one canonical discovered search-demand case for the first prerequisite-ready missing node, then let the normal native ownership and topic gates decide it. The allocator never creates the topic or article itself.
...
- Before any provider call, read executionPolicy in trend-topic-policy.yaml. While liveEnabled=true and the configured Paperclip policy permits the specific operation, use only the declared bounded live mode. If the policy is inactive, reuse durable reports and local inventory, continue independent non-trend portfolio tracks, and keep a lane-local monitor with nextReviewAt on that trend continuation. An inactive policy is expected operations state, not a technical owner blocker: do not retry, email, or send Telegram errors, and never substitute fixture output as production evidence.
...
function buildActivePluginConfig(pluginKey, oldConfig, secretIds) {
  ...
  if (pluginKey === "paperclip.semantic-core-mcp-agent-tools") {
    ...
    config.providerExecutionPolicy = "approved_candidate_batch";
...
- In a canonical topic-inventory refill, current typed case fields outrank stale linked issues and artifacts. When a named source lane has a future retry window and \`ownerActionRequired=false\`, keep the canonical refill in \`executing\` and PATCH its current CMO automation issue with \`executionPolicy.monitor.nextCheckAt=case.nextReviewAt\`, \`executionPolicy.monitor.scheduledBy=assignee\`, and concise notes. Do not resume because an older blocker issue became done. At monitor wake, resume only the named lane when eligible ready inventory or a permitted fresh/reused bounded run provides a concrete path; otherwise advance that lane monitor without comments or owner notifications. \`external_wait\` is reserved for a concrete board/user owner decision.
...
- After creating or linking work, re-read case-visible work products before deciding the stage. Positive completion proof routes to verify. A durable blocker artifact for a canonical topic-inventory refill stays in executing with the named lane monitor unless a real board/user owner decision has set ownerActionRequired=true; only then may it route to external_wait. Only a missing artifact may create or reuse one bounded evidence-recovery issue. Never build a recovery chain or leave a case in executing after a durable blocker is visible without either a named lane monitor or a delegated next source lane.
...
- An existing-owner outcome for a \`western_astrology_learning\` node is a non-article growth route, not curriculum supply. Once its stable growth action and delegation proof exist, mark the node as non-supply and resume the current canonical topic-inventory refill in the same management cycle. Delegate exactly one next prerequisite-ready missing curriculum node; do not wait for the refresh/reposition/merge implementation, create a duplicate article, or let this non-article action consume the bounded curriculum continuation.
...
- For trend validation and ingestion, a child being \`done\` is not success evidence. Require its source-case document at the delegated stable \`resultDocumentKey\`, then read the complete \`## trendIngestionResult\` JSON there and require \`finalDisposition\`, \`createdSearchDemandCaseIds\`, \`eligibleReadyTopicCountAfter\`, \`primaryAudienceSegmentId\`, \`segmentEligibleTopicCountAfter\`, \`remainingUnusedClusters\`, and \`validationWave\`. The bounded description projection is only wake context. The \`## trendIngestionResult\` heading is reserved for one machine-readable fenced JSON object only; do not use that exact heading for prose, checklists, or required-field notes. If accepted candidates exist but no search-demand case was created, delegate one ingestion handoff to SEO Blog Content Strategist; do not ask CTO to grant \`pipelines:write\` to semantic-core agents. If no search-demand case was created, the focused segment remains below five, the wave is below three, and eligible unused clusters remain, delegate the next bounded wave.
...
- When the campaign daily/run bound is exhausted and the next permitted focused report is on a future Kyiv window, keep the canonical refill case in \`executing\` with typed lane-local cooldown fields and a first-class monitor on its current CMO automation issue. Do not mark it measured, move it to external_wait, or ask the owner to invent topics.
...
- For a current-week \`topic_inventory_refill\` continuation, compute the portfolio-track deficits before reading a provider snapshot. When \`western_astrology_learning\` is deficient, the first bounded continuation is the approved curriculum graph: find the first prerequisite-ready missing node after checking consumed topics, delivered articles, and curriculum nodes already routed through a durable non-article existing-owner action. An existing owner route is not fulfilled or future supply: after its delegation proof exists, continue with the next prerequisite-ready missing node without waiting for the growth action implementation. Create or reuse exactly one canonical \`astrogen-search-demand-opportunities\` case at \`discovered\` with its curriculum fields and empty semantic arrays where no semantic proof exists. Continue the normal ownership, cannibalization, SERP, internal-link, Winning Structure and one-concept gates from there. A local Semantic Core snapshot with zero accepted keywords or clusters is not a no-topic result for this source lane, and never justifies \`external_wait\`, an owner request, or a direct topic/article create.
...
- Every curriculum topic and article must preserve \`curriculumNodeId\`, \`primaryConceptKey\`, exactly one matching item in \`introducedConceptKeys\`, prerequisite node/article links, excluded future concepts, the distinct teaching contribution, \`curriculumPublicationOrder\`, and \`prerequisitePublicationPlan\`. A dependent CMS draft may use a verified earlier prerequisite draft with a lower publication order, but only as a non-teaching planned canonical link. Its public release remains forbidden until every prerequisite URL is published. Previously taught concepts may be mentioned briefly but are not retaught.
...
- A curriculum prerequisite may proceed without accepted Semantic Core demand and without a classic SERP value gap only when SERP analysis is complete, the graph has a named missing node, no CMS owner or cannibalization conflict exists, incoming and outgoing prerequisite links are defined, and \`distinctTeachingContribution\` proves a unique teaching purpose. For CMS-draft readiness, every prerequisite must be published or be a verified earlier draft with a lower \`curriculumPublicationOrder\` and a durable publication plan; public release still requires published prerequisites. Winning Structure remains mandatory. More than one new concept, a missing eligible prerequisite plan, duplicate ownership, unsafe scope, or no distinct teaching role rejects it.
`;
  }
...
- Read each exact provider run's prepared \`paperclip_import.v1\` rows and decision trace. Review only the phrases named by the assigned \`trend-review:{trendReportRunId}:wave:{validationWave}\` fingerprint; never broaden the batch or invent alternatives.
...
- Submit decisions append-only through \`paperclip.semantic-core-mcp-agent-tools:submit-review-decisions\` using \`decision\` values \`accept\`, \`park\`, or \`reject\`. Partition reviewed phrases by original semantic run ID and intended non-core layer, then materialize each partition in exactly one \`run-layer-and-wait\` call with explicit \`mode=provider\`, \`provider_cache_mode=read_write\`, the same partition phrases, matching review decisions, and matching \`force_re_review_keywords\` records. This is permitted only by the explicit approved-candidate policy. Use \`read_only\` for cache-only rereads; \`refresh\` and \`bypass\` remain forbidden. Never combine different intended layers in one materializing run and do not rely on old runs mutating.
...
- Do not create or mutate native pipeline cases and do not request \`pipelines:write\`. Return the accepted evidence to the SEO Semantic Core Strategist, who delegates ingestion to SEO Blog Content Strategist. When the assigned issue is linked to a native source case, use the supplied \`sourceCaseId\` and \`resultDocumentKey\` to PUT the complete \`## trendIngestionResult\` projection into that source case document before setting the issue done. The linked-case-output document write is required handoff evidence, not permission to patch source case fields or stages. Keep the same bounded projection in the issue description for heartbeat context, but never rely on its truncated summary as the complete handoff. Your issue is complete only when final materialized dispositions and the source-case document are durable; downstream case mutation belongs to the parent validation wave.
`;
  }
...
- For \`western_astrology_learning\`, read /companies/astrogen/reference/western-astrology-curriculum.yaml and require one \`primaryConceptKey\` plus exactly one matching \`introducedConceptKeys\` item. A definition, list, comparison, FAQ answer, example, catalogue, or workflow for an adjacent astrology term is another concept even when labelled supporting context. For CMS-draft readiness, require published prerequisite URLs or verified earlier prerequisite drafts with lower \`curriculumPublicationOrder\` values and a durable \`prerequisitePublicationPlan\`; the later public release still requires published prerequisite URLs.
...
- Every bounded trend validation call must use \`project_id=astrogen-ukraine\`, 1-10 explicit \`candidate_keywords\`, one supported non-core layer, \`mode=provider\`, and \`provider_cache_mode=read_write\` only under the configured approved-candidate policy. A cache-only reread uses \`read_only\`; \`refresh\` and \`bypass\` are forbidden. An inactive Paperclip policy is a lane-local monitor inside the canonical refill's executing path, not permission to broaden the batch or to park the refill in external_wait. If the intended layer is ambiguous, return a typed handoff naming the ambiguity; do not guess a layer, retry the same invalid payload, or treat a portfolio-track label as a plugin enum.
- When a focused trend continuation is linked to a native source case, use the task-supplied \`sourceCaseId\` and \`resultDocumentKey\` to PUT the complete \`## focusedContinuationResult\` or \`## trendIngestionResult\` projection into that source case document before terminal done. Keep the bounded description projection too, but never treat a child summary or comment as the full handoff. This linked-case-output write does not authorize source case field or stage mutation.
...
- Call validate before start. A valid response must be remote proof: \`valid=true\`, \`validation_source=remote_mcp\`, \`input_hash\`, and \`task_input_contract_version\`. A run ID found only in \`evidenceRefs\`, a parent topic, a search-demand case, an opportunity-gate document, or another issue is upstream evidence, not a current article run. Reuse it only when plugin-returned input hash, namespace, and idempotency key exactly match the current \`winning-structure-input\`. Otherwise start exactly once for the current article task revision and idempotency key. If stale runtime indexes from an upstream run are present, preserve that run as provenance, clear only those indexes, and replace them with the current start response instead of blocking for manual repair. Store the returned input hash, preserve the current run ID, poll the same run with bounded backoff and stop on paused or terminal status.
...
- For \`western_astrology_learning\`, add a typed \`curriculumScope\` with one \`primaryConceptKey\`, exactly one matching \`introducedConceptKeys\` item, published or verified earlier-draft prerequisite URLs, \`curriculumPublicationOrder\`, \`prerequisitePublicationPlan\`, allowed known concepts and excluded future concepts. Remove any provider section that teaches an untaught term; calling it context does not make it acceptable. A planned prerequisite is a CMS-draft-only allowance and never permits public release out of sequence.
...
- Before any create call, read the accepted final slug and use \`payload_cms_find_blog_post\` once. If it returns the matching draft after a prior interrupted run, refetch and verify title, articleContent.v1, cover/OG media, CTA and relatedPosts, then write the returned CMS proof to the same case. Do not create a second post merely because Paperclip lost a post-write field mutation. Create only when no matching draft exists; never take over a published, approved, or mismatched draft.
