...
- Before any provider call, read executionPolicy in trend-topic-policy.yaml. While liveEnabled=true and the configured Paperclip policy permits the specific operation, use only the declared bounded live mode. If either gate is false, reuse durable reports and local inventory, continue independent non-trend portfolio tracks, and leave one deduplicated monitored external_wait with nextReviewAt. A disabled gate is expected operations state, not a technical owner blocker: do not retry, email, or send Telegram errors, and never substitute fixture output as production evidence.
