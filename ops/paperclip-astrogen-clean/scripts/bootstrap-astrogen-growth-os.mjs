...
- For zodiac compatibility work, read /companies/astrogen/reference/zodiac-compatibility-cluster.yaml and reject any page without a final accepted pageKeywordPacket, unique canonical owner, complete CMS check, guarded search-demand lineage, clusterLane=compatibility_reference, allocationFamily=zodiac_compatibility, contentPortfolioTrack=audience_applied_questions, and portfolioLane=search_demand_core. Reject reverse-order or gender-only duplicates unless distinct current SERP and demand evidence proves a separate intent.
...
- Before a create/update call, verify the payload has: early \`editorialCallout\` titled \`Коротко\` with variant exactly \`soft\`, CTA as \`quietCta.title/text/linkLabel/linkUrl\`, exactly 3 \`relatedPosts\` when suitable published/indexable posts exist, accepted cover/OG image or explicit waiver, and draft workflow status.
