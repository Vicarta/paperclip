...
- Before any provider call, read executionPolicy in trend-topic-policy.yaml. While liveEnabled=false, do not call generate-trend-topic-report or run-layer. Reuse durable reports and local inventory, continue independent non-trend portfolio tracks, and leave one deduplicated monitored external_wait with nextReviewAt. The provider lock is expected operations state, not a technical owner blocker: do not retry, email, or send Telegram errors, and never substitute fixture output as production evidence.
...
- Submit decisions append-only through \`paperclip.semantic-core-mcp-agent-tools:submit-review-decisions\` using \`decision\` values \`accept\`, \`park\`, or \`reject\`. Partition reviewed phrases by original semantic run ID and intended non-core layer, then materialize each partition in exactly one \`run-layer-and-wait\` call with explicit \`mode=provider\`, \`provider_cache_mode=read_only\`, the same partition phrases, matching review decisions, and matching \`force_re_review_keywords\` records. A cache miss or active provider lock becomes one monitored \`external_wait\`; \`read_write\`, \`refresh\`, and \`bypass\` require a separate operator-approved exact-candidate runbook. Never combine different intended layers in one materializing run and do not rely on old runs mutating.
...
- Every bounded trend validation call must use \`project_id=astrogen-ukraine\`, 1-10 explicit \`candidate_keywords\`, one supported non-core layer, \`mode=provider\`, and \`provider_cache_mode=read_only\`. A cache miss or active provider lock is a monitored external wait, not permission to retry, broaden the batch, or switch cache mode. If the intended layer is ambiguous, return a typed handoff naming the ambiguity; do not guess a layer, retry the same invalid payload, or treat a portfolio-track label as a plugin enum.
