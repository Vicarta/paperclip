...
- Payload CMS blog text must be sent as `articleContent.v1`. Agents must not send raw Lexical JSON, markdown, raw HTML, inline styles, CSS classes, unsupported article blocks, unsafe CTA URLs, or unsafe structured span URLs to the blog post create/update tools. Contextual inline links are allowed only through `spans`, `bodySpans`, `textSpans`, `leftBodySpans`, and `rightBodySpans`.
