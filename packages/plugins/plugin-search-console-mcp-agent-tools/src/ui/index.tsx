...
export function SearchConsoleMcpSettingsPage({ context }: PluginSettingsPageProps) {
  ...
  const [mcpUrl, setMcpUrl] = useState(DEFAULT_SEARCH_CONSOLE_MCP_URL);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setMcpUrl(
          typeof nextConfig.searchConsoleMcpUrl === "string" &&
            nextConfig.searchConsoleMcpUrl.trim().length > 0
            ? nextConfig.searchConsoleMcpUrl
            : DEFAULT_SEARCH_CONSOLE_MCP_URL,
        );
        setAllowedSiteUrl(
          ...
          setMessage({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Failed to load Search Console MCP plugin settings.",
          });
        }
      } finally {
        if (!cancelled) setLoading(false);
  ...
  const currentSecret = secrets.find(
    (secret: CompanySecret) => secret.id === config.searchConsoleMcpTokenSecretRef,
  );
  ...
  async function handleSave() {
    ...
    try {
      let secretId = config.searchConsoleMcpTokenSecretRef ?? "";
        ...
        if (secretId) {
          ...
          const created = await api<CompanySecret>(
            `/companies/${companyId}/secrets`,
            {
              method: "POST",
              body: JSON.stringify({
                name: "Search Console MCP Astrogen Token",
                description:
                  "Bearer token for the private Astrogen Search Console MCP endpoint.",
                value: trimmedToken,
              }),
            },
          );
      ...
      const nextConfig: PluginConfig = {
        searchConsoleMcpTokenSecretRef: secretId || "",
        searchConsoleMcpUrl: mcpUrl.trim() || DEFAULT_SEARCH_CONSOLE_MCP_URL,
      ...
      setMessage({
        tone: "success",
        message: "Search Console MCP plugin settings saved.",
      });
    } catch (error) {
      setMessage({
        tone: "error",
        message:
          error instanceof Error
            ? error.message
            : "Failed to save Search Console MCP plugin settings.",
      });
    } finally {
      setSaving(false);
    }
  ...
  if (!companyId) {
    return (
      <div style={cardStyle}>
        Select a company to configure the Search Console MCP plugin.
      </div>
    );
  }

  if (loading) {
    return <div style={cardStyle}>Loading Search Console MCP plugin settings...</div>;
  }
        ...
        <div style={{ display: "grid", gap: "12px" }}>
          <div>
            ...
            <input
              ...
              placeholder={DEFAULT_SEARCH_CONSOLE_MCP_URL}
            />
          ...
          <div>
            <label style={labelStyle}>Allowed Search Console site</label>
            <input
              style={inputStyle}
          ...
          <div>
            ...
            {config.searchConsoleMcpTokenSecretRef && !replaceToken ? (
              ...
              <div style={{ display: "grid", gap: "8px" }}>
                <input
                  ...
                  placeholder={
                    config.searchConsoleMcpTokenSecretRef
