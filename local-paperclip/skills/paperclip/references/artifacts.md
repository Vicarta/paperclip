...
## Execution Workspace Artifacts

When the deliverable file already exists in the current execution workspace,
prefer the typed server registration route. This is the canonical harness path
for generated images, reports, bundles, and other files created during a run:

```bash
curl -sS -X POST \
  "$PAPERCLIP_API_URL/api/issues/$PAPERCLIP_TASK_ID/work-products/register-workspace-artifact" \
  -H "Authorization: Bearer $PAPERCLIP_API_KEY" \
  -H "X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID" \
  -H "Content-Type: application/json" \
  --data-binary '{
    "relativePath": "work/output/cover.png",
    "title": "Accepted cover image",
    "summary": "QA-passed cover image generated for this issue.",
...
The route validates the path against the issue execution workspace, stores the
file as an issue attachment, and creates the canonical attachment-backed
`provider=paperclip`, `type=artifact` work product. It is idempotent for the
same issue/workspace/path. Do not handcraft raw `provider=paperclip` artifact
work products for execution workspace files.
