...
describe("Better Auth cookie scoping", () => {
  it("does not rewrite reverse-proxy auth URLs to the internal listen port", () => {
    expect(rewriteLocalAuthUrlPort("https://ubuntu-oc.tailbd4e1c.ts.net:4447", 3100)).toBe(
      "https://ubuntu-oc.tailbd4e1c.ts.net:4447",
    );
    expect(rewriteLocalAuthUrlPort("https://paperclip.example.test:4447", 3100)).toBe(
      "https://paperclip.example.test:4447",
    );
  });

  it("rewrites local auth URLs when a dev server selects a different port", () => {
    expect(rewriteLocalAuthUrlPort("http://127.0.0.1:3100", 3110)).toBe("http://127.0.0.1:3110/");
    expect(rewriteLocalAuthUrlPort("http://localhost:3100", 3110)).toBe("http://localhost:3110/");
  });
