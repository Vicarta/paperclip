...
describeEmbeddedPostgres("routine routes end-to-end", () => {
  ...
  it("runs routines with variable inputs and interpolates the execution issue description", async () => {
    ...
    const heartbeatContextRes = await request(app).get(
      `/api/issues/${runRes.body.linkedIssueId}/heartbeat-context`,
    );
    expect(heartbeatContextRes.status).toBe(200);
    expect(heartbeatContextRes.body.routineRun).toMatchObject({
      id: runRes.body.id,
      source: "manual",
