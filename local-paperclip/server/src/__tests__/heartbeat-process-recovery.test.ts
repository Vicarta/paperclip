...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("converts an exhausted successful handoff with delegated child work into a dependency wait", async () => {
    const { companyId, agentId, runId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "succeeded",
      livenessState: "advanced",
    ...
    await expect(sourceBlockerIssueIds(companyId, issueId)).resolves.toEqual([delegatedChildId]);
    ...
    const comments = await db.select().from(issueComments).where(eq(issueComments.issueId, issueId));
    expect(comments).toHaveLength(1);
    expect(comments[0]?.body).toContain("delegated the next step to open child work");
  ...
  it("dispatches assigned todo work when an old queued wake has no runnable heartbeat run", async () => {
    const { companyId, agentId, issueId } = await seedAssignedTodoNoRunFixture();
    await db.insert(agentWakeupRequests).values({
      companyId,
      agentId,
      ...
      payload: { issueId },
      status: "queued",
      requestedAt: new Date("2026-03-19T00:00:00.000Z"),
    });
    const heartbeat = heartbeatService(db);
    ...
    const runs = await db.select().from(heartbeatRuns).where(eq(heartbeatRuns.agentId, agentId));
    expect(runs).toHaveLength(1);
    expect(runs[0]?.wakeupRequestId).toBeTruthy();
    expect(runs[0]?.contextSnapshot as Record<string, unknown>).toMatchObject({
      issueId,
      taskId: issueId,
      wakeReason: "issue_assigned",
      source: "issue.assigned_todo_liveness_dispatch",
