...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  it("returns child handoff projection without letting system notices hide agent evidence", async () => {
    const companyId = randomUUID();
    const assigneeAgentId = randomUUID();
    await db.insert(companies).values({
      id: companyId,
    ...
    await db.insert(issueComments).values([
      {
        ...
        companyId,
        issueId: childA,
        body: "Agent handoff evidence for child A.",
        authorType: "agent",
      ...
      },
      {
        ...
        authorType: "system",
        createdAt: new Date("2026-07-06T10:05:00Z"),
        updatedAt: new Date("2026-07-06T10:05:00Z"),
      },
    ]);

    expect(await svc.getChildHandoffSummary(parentId)).toMatchObject({
      childIssueIds: [childA, childB],
      childIssueSummaries: [
        expect.objectContaining({
          id: childA,
          status: "done",
          summary: "Agent handoff evidence for child A.",
        }),
        expect.objectContaining({
          id: childB,
          status: "in_progress",
