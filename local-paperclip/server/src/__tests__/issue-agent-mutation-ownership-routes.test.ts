...
function createRunContextDb(
  ...
  const runCreatedIssueIds = Array.isArray(contextSnapshot.runCreatedIssueIds)
    ? contextSnapshot.runCreatedIssueIds.filter((entry): entry is string => typeof entry === "string")
    : [];
  const rowsForSelection = (selection: Record<string, unknown>) => {
    ...
    if (keys.includes("createdByAgentId") && keys.includes("originRunId")) {
      return runCreatedIssueIds.map((id) => ({
        id,
        companyId,
        createdByAgentId: runAgentId,
        originRunId: runId,
      }));
    }
...
describe("agent issue mutation checkout ownership", () => {
  ...
  it("lets a pinned manager run route an existing backlog issue through tasks:assign", async () => {
    const sourceIssueId = "99999999-9999-4999-8999-999999999999";
    const decide = vi.fn(async (input: { action: string }) => ({
      allowed: input.action === "tasks:assign" || input.action === "company_scope:read",
      action: input.action,
      reason: input.action === "tasks:assign" || input.action === "company_scope:read"
        ? "allow_explicit_grant"
        : "deny_missing_grant",
      explanation: input.action === "tasks:assign" || input.action === "company_scope:read"
        ? "Allowed by test routing grant."
        : "Pinned manager does not get broad issue mutation.",
    }));
    (mockAccessService as any).decide = decide;
    mockIssueService.getById.mockResolvedValue(makeIssue({
      status: "backlog",
    ...
    const app = await createApp(
      ownerActor(),
      createRunContextDb({ issueId: sourceIssueId }, ownerAgentId, ownerRunId),
    );
    const res = await request(app)
      .patch(`/api/issues/${issueId}`)
    ...
    expect(decide).not.toHaveBeenCalledWith(expect.objectContaining({ action: "issue:mutate" }));
    expect(mockHeartbeatService.wakeup).toHaveBeenCalledWith(
      peerAgentId,
      expect.objectContaining({
  ...
  it("does not let pinned manager routing bypass broad issue mutation", async () => {
    const sourceIssueId = "99999999-9999-4999-8999-999999999999";
    mockAccessService.decide.mockImplementation(async (input: { action: string }) => ({
      allowed: input.action === "tasks:assign" || input.action === "company_scope:read",
      action: input.action,
      reason: input.action === "tasks:assign" || input.action === "company_scope:read"
        ? "allow_explicit_grant"
        : "deny_missing_grant",
      explanation: "Test broad mutation denial.",
    }));
    mockIssueService.getById.mockResolvedValue(makeIssue({
      status: "backlog",
      assigneeAgentId: null,
    ...
    const res = await request(app)
      .patch(`/api/issues/${issueId}`)
      .send({ assigneeAgentId: peerAgentId, status: "todo", title: "Too broad" });

    expect(res.status).toBe(403);
    expect(res.body.error).toBe("Agent run is pinned to a different issue");
    expect(mockIssueService.update).not.toHaveBeenCalled();
  });

  it("lets a pinned run create children under an issue it created in the same run", async () => {
    const sourceIssueId = "99999999-9999-4999-8999-999999999999";
    mockIssueService.getById.mockResolvedValue(makeIssue({
      status: "backlog",
      assigneeAgentId: null,
    ...
    const app = await createApp(
      ownerActor(),
      createRunContextDb({ issueId: sourceIssueId, runCreatedIssueIds: [issueId] }, ownerAgentId, ownerRunId),
    );
    const res = await request(app)
      .post(`/api/issues/${issueId}/children`)
      .send({
        title: "Next article stage",
      });
