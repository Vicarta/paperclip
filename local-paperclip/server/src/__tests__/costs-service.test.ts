...
describeEmbeddedPostgres("cost and finance aggregate overflow handling", () => {
  ...
  it("aggregates sub-cent amount micros instead of truncating plugin costs", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    ...
    expect(summary.spendCents).toBe(0.35);
    expect(byAgentRow?.costCents).toBe(0.35);
    expect(byProviderRow?.costCents).toBe(0.35);
    expect(byProjectRow?.costCents).toBe(0.35);
  });
