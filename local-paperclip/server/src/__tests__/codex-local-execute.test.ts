...
const payload = {
  ...
  paperclipProjectId: process.env.PAPERCLIP_PROJECT_ID || null,
...
describe("codex execute", () => {
  ...
  it("falls back to a fresh session when the saved Codex session log is not writable", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-execute-unwritable-session-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const paperclipHome = path.join(root, "paperclip-home");
    ...
    const sessionPath = path.join(
      ...
      "07",
      "06",
      "rollout-2026-07-06T16-47-47-codex-session-1.jsonl",
    );
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(path.dirname(sessionPath), { recursive: true });
    await fs.writeFile(sessionPath, "{}\n", { mode: 0o400 });
    await fs.chmod(sessionPath, 0o400);
    await writeFakeCodexCommand(commandPath);
    ...
    try {
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Skipping saved Codex session \"codex-session-1\""),
        }),
      );
      expect(invocationNotes).toContain(
        "Skipped saved Codex session codex-session-1 because its session log was not writable; started a fresh session from Paperclip heartbeat context.",
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    ...
    try {
      ...
      expect(capture.paperclipProjectId).toBe("project-1");
