...
describe.sequential("plugin tool and bridge authz", () => {
  ...
  it("keeps the legacy agent plugin-tools discovery route working", async () => {
    const listToolsForAgent = vi.fn(() => []);
    const { app } = await createApp(agentActor(), {}, {
      toolDeps: {
        toolDispatcher: {
    ...
    const res = await request(app).get("/api/agents/me/plugin-tools?pluginId=paperclip.example");

    expect(res.status).toBe(200);
    expect(listToolsForAgent).toHaveBeenCalledWith({ pluginId: "paperclip.example" });
  });
  ...
  it("keeps the legacy agent plugin-tools execute route working", async () => {
    const executeTool = vi.fn().mockResolvedValue({ content: "ok" });
    const { app } = await createApp(
      agentActor(),
      {},
  ...
  it("accepts common agent tool execute aliases while forwarding canonical parameters", async () => {
    const executeTool = vi.fn().mockResolvedValue({ content: "ok" });
    const { app } = await createApp(
      agentActor(),
      {},
    ...
    const res = await request(app)
      .post("/api/agents/me/plugin-tools/execute")
      .send({
        toolName: "paperclip.example:search",
        params: { q: "test" },
        runContext: { agentId: agentA, runId: runA, companyId: companyA, projectId: projectA },
      });
