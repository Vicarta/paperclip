...
import { startCostMonthlyRollupRefresh } from "./services/costs.js";
import { startRuntimeRetention } from "./services/runtime-retention.js";
import { buildRuntimeApiCandidateUrls, choosePrimaryRuntimeApiUrl } from "./runtime-api.js";
import { createPluginWorkerManager } from "./services/plugin-worker-manager.js";
import { createStorageServiceFromConfig } from "./storage/index.js";
import { rewriteLocalAuthUrlPort } from "./auth/public-url.js";
...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.authBaseUrlMode === "explicit" && config.authPublicBaseUrl) {
    config.authPublicBaseUrl = rewriteLocalAuthUrlPort(config.authPublicBaseUrl, listenPort);
  }
  ...
  if (config.costRollupEnabled) {
    startCostMonthlyRollupRefresh(db as any, { monthCount: 2 });
  }

  if (config.runtimeRetentionEnabled) {
    startRuntimeRetention(db as any, {
      retentionDays: config.runtimeRetentionDays,
      runLogRetentionDays: config.runLogRetentionDays,
      runLogCompressAfterHours: config.runLogCompressAfterHours,
    });
  }
