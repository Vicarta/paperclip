...
const registerWorkspaceArtifactSchema = z.object({
  relativePath: z.string().trim().min(1).max(2_000),
  title: z.string().trim().min(1).max(200),
  summary: z.string().trim().max(8_000).optional().nullable(),
  contentType: z.string().trim().min(1).max(200).optional().nullable(),
  status: z.enum([
    "active",
    "ready_for_review",
    "approved",
    ...
    "archived",
    "draft",
  ]).optional().default("ready_for_review"),
  reviewState: z.enum(["none", "needs_board_review", "approved", "changes_requested"]).optional().default("none"),
...
export function issueRoutes(
  ...
  const AGENT_TASK_ROUTING_PATCH_KEYS = new Set(["assigneeAgentId", "comment", "status"]);
  const AGENT_TASK_ROUTING_STATUSES = new Set(["backlog", "todo"]);

  function isAgentTaskRoutingOnlyPatch(body: unknown) {
    if (!body || typeof body !== "object" || Array.isArray(body)) return false;
    const keys = Object.keys(body);
    if (keys.length === 0) return false;
    if (!keys.every((key) => AGENT_TASK_ROUTING_PATCH_KEYS.has(key))) return false;
    return Object.prototype.hasOwnProperty.call(body, "assigneeAgentId");
  }

  async function assertAgentTaskRoutingPatchAllowed(
    req: Request,
    issue: {
      id: string;
      companyId: string;
    ...
    if (req.actor.type !== "agent") return false;
    if (!isAgentTaskRoutingOnlyPatch(req.body)) return false;
    if (input.assigneeAgentId === undefined || input.assigneeAgentId === null) return false;
    if (input.status !== undefined && !AGENT_TASK_ROUTING_STATUSES.has(input.status)) return false;
  ...
  type PinnedRunIssueAction = "issue:read" | "issue:heartbeat_context" | "issue:checkout" | "issue:comment" | "issue:mutate";

  function readPinnedRunIssueId(contextSnapshot: unknown): string | null {
    ...
    const paperclipIssue = context.paperclipIssue && typeof context.paperclipIssue === "object" && !Array.isArray(context.paperclipIssue)
      ? context.paperclipIssue as Record<string, unknown>
      : null;
    return readNonEmptyString(context.issueId) ?? readNonEmptyString(context.taskId) ?? readNonEmptyString(paperclipIssue?.id);
  }

  function hasExplicitIssueReroute(contextSnapshot: unknown, pinnedIssueId: string, targetIssueId: string): boolean {
    if (!contextSnapshot || typeof contextSnapshot !== "object" || Array.isArray(contextSnapshot)) return false;
    const context = contextSnapshot as Record<string, unknown>;
    const reroute = context.issueReroute;
    if (!reroute || typeof reroute !== "object" || Array.isArray(reroute)) return false;
    const record = reroute as Record<string, unknown>;
    const fromIssueId = readNonEmptyString(record.fromIssueId) ?? readNonEmptyString(record.sourceIssueId);
    const toIssueId = readNonEmptyString(record.toIssueId) ?? readNonEmptyString(record.targetIssueId) ?? readNonEmptyString(record.issueId);
    const traceId =
      readNonEmptyString(record.traceId) ??
      readNonEmptyString(record.activityId) ??
      readNonEmptyString(record.approvalId) ??
      readNonEmptyString(record.commentId);
    return fromIssueId === pinnedIssueId && toIssueId === targetIssueId && !!traceId;
  }

  async function assertPinnedRunIssueTarget(
    req: Request,
    res: Response,
    issue: { id: string; companyId: string },
    action: PinnedRunIssueAction,
  ) {
    if (req.actor.type !== "agent") return true;
    const actorAgentId = req.actor.agentId;
    const runId = req.actor.runId?.trim();
    if (!actorAgentId || !runId) return true;
    const run = await db
      .select({
        agentId: heartbeatRuns.agentId,
        companyId: heartbeatRuns.companyId,
      ...
      .then((rows) => rows[0] ?? null);
    if (!run || run.agentId !== actorAgentId || run.companyId !== issue.companyId) return true;

    const pinnedIssueId = readPinnedRunIssueId(run.contextSnapshot);
    if (!pinnedIssueId || pinnedIssueId === issue.id) return true;
    if (hasExplicitIssueReroute(run.contextSnapshot, pinnedIssueId, issue.id)) return true;
    const runCreatedIssue = await db
      .select({
        createdByAgentId: issueRows.createdByAgentId,
        originRunId: issueRows.originRunId,
      })
      .from(issueRows)
      .where(and(eq(issueRows.id, issue.id), eq(issueRows.companyId, issue.companyId)))
      .then((rows) => rows[0] ?? null);
    if (runCreatedIssue?.createdByAgentId === actorAgentId && runCreatedIssue.originRunId === runId) return true;

    res.status(403).json({
      error: "Agent run is pinned to a different issue",
      details: {
        action,
        issueId: issue.id,
        pinnedIssueId,
  ...
  async function assertIssueReadAllowed(
    ...
    action: PinnedRunIssueAction = "issue:read",
  ) {
    if (!(await assertPinnedRunIssueTarget(req, res, issue, action))) return false;
  ...
  async function assertAgentIssueCommentAllowed(
    ...
    if (!(await assertPinnedRunIssueTarget(req, res, issue, "issue:comment"))) return false;
  ...
  async function assertAgentIssueMutationAllowed(
    ...
    if (!(await assertPinnedRunIssueTarget(req, res, issue, "issue:mutate"))) return false;
  ...
  router.get("/issues/:id/heartbeat-context", async (req, res) => {
    ...
    if (!(await assertIssueReadAllowed(req, res, issue, "issue:heartbeat_context"))) return;
    ...
    const [
      ...
      await Promise.all([
        ...
        svc.getChildHandoffSummary(issue.id),
        issue.originKind === "routine_execution" && issue.originRunId
          ? db
              .select({
                id: routineRuns.id,
    ...
    res.json({
      ...
      continuationSummary: safeContinuationSummary
        ...
        : null,
      childWork: childHandoffSummary,
      routineRun: routineRunContext
        ? {
            id: routineRunContext.id,
            source: routineRunContext.source,
            status: routineRunContext.status,
            triggeredAt: routineRunContext.triggeredAt,
            idempotencyKey: routineRunContext.idempotencyKey,
            triggerPayload: routineRunContext.triggerPayload ?? null,
          }
        : null,
      currentExecutionWorkspace,
    });
  ...
  router.post("/issues/:id/work-products/register-workspace-artifact", validate(registerWorkspaceArtifactSchema), async (req, res) => {
    const id = req.params.id as string;
    const issue = await svc.getById(id);
    if (!issue) {
      res.status(404).json({ error: "Issue not found" });
    ...
    const relativePath = normalizeWorkspaceArtifactRelativePath(req.body.relativePath);
    const executionWorkspaceId = typeof issue.executionWorkspaceId === "string" ? issue.executionWorkspaceId : null;
    const issueProjectWorkspaceId = typeof issue.projectWorkspaceId === "string" ? issue.projectWorkspaceId : null;
    const workspace = await (async (): Promise<{
      scopeId: string;
      scopeKind: "execution_workspace" | "project_workspace" | "project_default_workspace";
      projectId: string | null;
      executionWorkspaceId: string | null;
      root: string;
    } | null> => {
      if (executionWorkspaceId) {
        const row = await db
          ...
          .from(executionWorkspaces)
          .where(and(eq(executionWorkspaces.id, executionWorkspaceId), eq(executionWorkspaces.companyId, issue.companyId)))
          .then((rows) => rows[0] ?? null);
        if (!row) {
          throw unprocessable("Issue execution workspace was not found", { code: "execution_workspace_not_found" });
        }
        if (row.providerType !== "local_fs" && row.providerType !== "git_worktree") {
          throw unprocessable("Workspace artifact registration requires a local execution workspace", {
            code: "unsupported_execution_workspace_provider",
          });
        }
        const root = row.cwd ?? row.providerRef ?? null;
        if (!root) {
          throw unprocessable("Execution workspace has no local path", { code: "execution_workspace_path_missing" });
        }
        return {
          scopeId: row.id,
      ...
      const projectWorkspace = issueProjectWorkspaceId
        ? await db
          .select({
          ...
          .from(projectWorkspaces)
          .where(and(eq(projectWorkspaces.id, issueProjectWorkspaceId), eq(projectWorkspaces.companyId, issue.companyId)))
          .then((rows) => rows[0] ?? null)
        : issue.projectId
          ? await db
            .select({
          ...
          : null;
      if (projectWorkspace) {
        if (projectWorkspace.sourceType !== "local_path") {
          throw unprocessable("Workspace artifact registration requires a local project workspace", {
            code: "unsupported_project_workspace_source",
          });
        }
        if (!projectWorkspace.cwd) {
          throw unprocessable("Project workspace has no local path", { code: "project_workspace_path_missing" });
        }
        return {
          scopeId: projectWorkspace.id,
          scopeKind: "project_workspace",
          projectId: projectWorkspace.projectId,
          executionWorkspaceId: null,
          root: projectWorkspace.cwd,
        };
      }

      if (issue.projectId) {
        const instanceRoot = process.env.PAPERCLIP_HOME && process.env.PAPERCLIP_INSTANCE_ID
          ? path.join(
        ...
        if (instanceRoot) {
          try {
            const stat = await fs.stat(instanceRoot);
            if (stat.isDirectory()) {
              return {
                scopeId: `project-default:${issue.projectId}`,
                scopeKind: "project_default_workspace",
                projectId: issue.projectId,
                executionWorkspaceId: null,
                root: instanceRoot,
              };
    ...
    if (!workspace) {
      throw unprocessable("Issue has no local workspace for workspace artifact registration", {
        code: "workspace_required",
      });
    }

    const externalId = buildWorkspaceArtifactExternalId(workspace.scopeId, relativePath);
    const existing = (await workProductsSvc.listForIssue(issue.id)).find(
      (product) =>
        product.type === "artifact" &&
        product.provider === "paperclip" &&
        product.externalId === externalId,
    );
    if (existing) {
      const attachmentId = typeof existing.metadata?.attachmentId === "string" ? existing.metadata.attachmentId : null;
      const attachment = attachmentId ? await svc.getAttachmentById(attachmentId) : null;
      res.status(200).json({
        existing: true,
        attachment: attachment ? withContentPath(attachment) : null,
        workProduct: existing,
      });
      return;
    }

    const rootRealPath = await fs.realpath(workspace.root);
    const absoluteCandidate = path.resolve(rootRealPath, relativePath);
    let fileRealPath: string;
    try {
      fileRealPath = await fs.realpath(absoluteCandidate);
    } catch {
      throw unprocessable("Workspace artifact file was not found", {
        code: "workspace_artifact_not_found",
      });
    }
    if (!isPathInsideDirectory(rootRealPath, fileRealPath)) {
      throw unprocessable("Workspace artifact path escapes the execution workspace", {
    ...
    if (!stat.isFile()) {
      throw unprocessable("Workspace artifact path must point to a file", {
        code: "workspace_artifact_not_file",
      });
    }
    ...
    if (stat.size <= 0) {
      throw unprocessable("Workspace artifact file is empty", { code: "workspace_artifact_empty" });
    }
    if (stat.size > attachmentMaxBytes) {
      throw unprocessable(`Workspace artifact exceeds ${attachmentMaxBytes} bytes`, {
        code: "workspace_artifact_too_large",
      });
    }
    ...
    const stored = await storage.putFile({
      companyId: issue.companyId,
      namespace: `issues/${issue.id}/workspace-artifacts`,
      originalFilename,
      contentType: inferWorkspaceArtifactContentType(relativePath, req.body.contentType ?? null),
      body: await fs.readFile(fileRealPath),
    });
    ...
    const metadata = await canonicalizePaperclipArtifactMetadata({
      issue,
      metadata: { attachmentId: attachment.id },
    });
    const product = await workProductsSvc.createForIssue(issue.id, issue.companyId, {
      projectId: issue.projectId ?? workspace.projectId ?? null,
    ...
    await logActivity(db, {
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "issue.workspace_artifact_registered",
      entityType: "issue",
      entityId: issue.id,
      details: {
        attachmentId: attachment.id,
        ...
        workspaceKind: workspace.scopeKind,
      },
    });
    await revalidateActiveSourceRecoveryAfterCommittedWrite({
      issue,
  ...
  router.post("/companies/:companyId/issues", applyCreateIssueStatusDefault, validate(createIssueSchema), async (req, res) => {
    ...
    const createBody = {
      ...
      ...(actor.actorType === "agent" && actor.runId ? { originRunId: actor.runId } : {}),
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    const agentTaskRoutingPatchAllowed = await assertAgentTaskRoutingPatchAllowed(req, existing, {
      assigneeAgentId: normalizedAssigneeAgentId,
      status: typeof req.body.status === "string" ? req.body.status : undefined,
    });
    if (!agentTaskRoutingPatchAllowed && !(await assertAgentIssueMutationAllowed(req, res, existing))) return;
        ...
        for (const dependent of dependents) {
          addWakeup(dependent.assigneeAgentId, {
            ...
            payload: {
              ...
              resolvedBlockerSummaries: dependent.resolvedBlockerSummaries,
            },
            requestedByActorType: actor.actorType,
            requestedByActorId: actor.actorId,
            contextSnapshot: {
  ...
  router.post("/issues/:id/checkout", validate(checkoutIssueSchema), async (req, res) => {
    ...
    if (!(await assertPinnedRunIssueTarget(req, res, issue, "issue:checkout"))) return;
