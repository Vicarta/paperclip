...
export type IssueCompletionHandoffSummary = ChildIssueCompletionSummary;
...
async function listIssueCompletionHandoffSummaries(
  dbOrTx: Db,
  companyId: string,
  issueIds: string[],
): Promise<IssueCompletionHandoffSummary[]> {
  const uniqueIssueIds = [...new Set(issueIds.filter(Boolean))].slice(0, MAX_CHILD_COMPLETION_SUMMARIES);
  ...
  const commentRows = issueRows.length > 0
    ? await dbOrTx
        .select({
        ...
        .where(and(
          ...
          inArray(issueComments.issueId, issueRows.map((issue) => issue.id)),
          isNull(issueComments.deletedAt),
        ))
        .orderBy(desc(issueComments.createdAt), desc(issueComments.id))
    : [];
  const latestCommentByIssueId = new Map<string, string>();
  const latestNonSystemCommentByIssueId = new Map<string, string>();
  for (const comment of commentRows) {
    if (!latestCommentByIssueId.has(comment.issueId)) {
      latestCommentByIssueId.set(comment.issueId, comment.body);
    }
    if (comment.authorType !== "system" && !latestNonSystemCommentByIssueId.has(comment.issueId)) {
      latestNonSystemCommentByIssueId.set(comment.issueId, comment.body);
    }
  }
  ...
  return issueRows.map((issue) => ({
    ...
    summary: truncateInlineSummary(
      latestNonSystemCommentByIssueId.get(issue.id) ?? latestCommentByIssueId.get(issue.id),
    ),
  }));
}
...
type IssueBlockerAttentionInputNode =
  ...
  >
  & { checkoutRunId?: string | null; executionRunId?: string | null };
...
function withActiveRuns(
  ...
  return issueRows.map((row) => ({
    ...row,
    activeRun:
      (row.executionRunId ? runMap.get(row.executionRunId) : null) ??
      (row.checkoutRunId ? runMap.get(row.checkoutRunId) : null) ??
      null,
  }));
}
...
export function issueService(db: Db) {
  ...
  return {
    ...
    listWakeableBlockedDependents: async (blockerIssueId: string) => {
      ...
      const readyDependents = wakeableCandidates
        .map((candidate) => {
      ...
      return Promise.all(readyDependents.map(async ({ candidate, readiness }) => ({
          id: candidate.id,
          assigneeAgentId: candidate.assigneeAgentId!,
          blockerIssueIds: readiness.blockerIssueIds,
          resolvedBlockerSummaries: await listIssueCompletionHandoffSummaries(
            db,
            blockerIssue.companyId,
            readiness.blockerIssueIds,
          ),
        })));
    },
    ...
    getWakeableParentAfterChildCompletion: async (parentIssueId: string) => {
      ...
      const childIssueSummaries = await listIssueCompletionHandoffSummaries(
        db,
        parent.companyId,
        children.slice(0, MAX_CHILD_COMPLETION_SUMMARIES).map((child) => child.id),
      );
    ...
    getChildHandoffSummary: async (parentIssueId: string) => {
      const parent = await db
        .select({
          id: issues.id,
          companyId: issues.companyId,
      ...
      const terminalChildCount = children.filter((child) => child.status === "done" || child.status === "cancelled").length;

      return {
        childIssueIds: children.map((child) => child.id),
        childIssueSummaries,
        childIssueSummaryTruncated: children.length > childIssueSummaries.length,
        openChildCount: children.length - terminalChildCount,
        terminalChildCount,
      };
    },
