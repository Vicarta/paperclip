...
export const QUEUED_ISSUE_WAKE_RUN_LINK_GRACE_MS = Math.max(
  60_000,
  Number(process.env.QUEUED_ISSUE_WAKE_RUN_LINK_GRACE_MS) || 5 * 60 * 1000,
);
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function hasQueuedIssueWake(companyId: string, issueId: string) {
    const runLinkGraceCutoff = new Date(Date.now() - QUEUED_ISSUE_WAKE_RUN_LINK_GRACE_MS);
    ...
    return db
      ...
      .where(
        and(
          ...
          sql`${agentWakeupRequests.payload} ->> 'issueId' = ${issueId}`,
          sql`(
            ${agentWakeupRequests.requestedAt} > ${runLinkGraceCutoff.toISOString()}::timestamptz
            or exists (
              select 1
              from heartbeat_runs linked_run
              where linked_run.company_id = ${agentWakeupRequests.companyId}
                and linked_run.wakeup_request_id = ${agentWakeupRequests.id}
                and linked_run.status in (${sql.join([...EXECUTION_PATH_HEARTBEAT_RUN_STATUSES].map((status) => sql`${status}`), sql`, `)})
            )
          )`,
        ),
      )
  ...
  async function resolveIssueWaitingOnOpenDependencies(
    issue: typeof issues.$inferSelect,
    input: {
      reasonCopy: string;
      source: string;
    ...
    const openChildIds = new Set(openChildren.map((row) => row.id));
    const trueBlockers = existingBlockers.filter((row) => !openChildIds.has(row.id));
    const waitingIssueIds = [...new Set([...trueBlockers.map((row) => row.id), ...openChildren.map((row) => row.id)])];
    if (waitingIssueIds.length === 0) return null;

    const nextStatus = trueBlockers.length > 0 ? "blocked" : "in_progress";
    const blockedByIssueIds = trueBlockers.map((row) => row.id);
    const updated = await issuesSvc.update(issue.id, { status: nextStatus, blockedByIssueIds });
    if (!updated) return null;

    const waitingOn = formatIssueLinksForComment([...openChildren, ...trueBlockers]);
    await issuesSvc.addComment(
      issue.id,
    ...
    await logActivity(db, {
      ...
      details: {
        ...
        source: input.source,
        blockedByIssueIds,
        waitingOnIssueIds: waitingIssueIds,
      },
    });
    return updated;
  }
  ...
  async function resolveContinuationWaitingOnReview(issue: typeof issues.$inferSelect) {
    return resolveIssueWaitingOnOpenDependencies(issue, {
  ...
  async function resolveSuccessfulRunHandoffWaitingOnDelegatedWork(issue: typeof issues.$inferSelect) {
    return resolveIssueWaitingOnOpenDependencies(issue, {
      source: "recovery.reconcile_successful_run_handoff_delegated_work",
      reasonCopy:
        "(The latest successful handoff delegated the next step to open child work; " +
        "Paperclip turned that into a normal dependency wait instead of treating the parent as missing a disposition.)",
    });
  }
  ...
  async function reconcileStrandedAssignedIssues() {
    ...
    for (const issue of candidates) {
      ...
      if (handoffEvidence) {
        ...
        const delegatedWait = await resolveSuccessfulRunHandoffWaitingOnDelegatedWork(issue);
        if (delegatedWait) {
