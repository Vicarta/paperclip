...
describe("renderPaperclipWakePrompt", () => {
  it("keeps the default local-agent prompt action-oriented", () => {
    ...
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("Agent plugin tools");
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("POST /api/agents/me/plugin-tools/execute");
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("Use `parameters` as the canonical payload key");
  ...
  it("includes continuation and child issue summaries in structured wake context", () => {
      ...
      resolvedBlockerSummaries: [
        {
          ...
          status: "done",
          priority: "medium",
          summary: "ready_for_brief_creation=yes; top topic selected.",
        },
      ],
    };
    ...
    expect(prompt).toContain("PAP-099 Select topic (done)");
    expect(prompt).toContain("ready_for_brief_creation=yes; top topic selected.");
  });
});
