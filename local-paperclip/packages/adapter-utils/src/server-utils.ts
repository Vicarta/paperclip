...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Agent plugin tools: discover them with GET /api/agents/me/plugin-tools. Execute exactly one tool with POST /api/agents/me/plugin-tools/execute using body {tool, parameters, runContext:{agentId:PAPERCLIP_AGENT_ID, runId:PAPERCLIP_RUN_ID, companyId:PAPERCLIP_COMPANY_ID, projectId:PAPERCLIP_PROJECT_ID, issueId:PAPERCLIP_TASK_ID}}. Use `parameters` as the canonical payload key; create JSON with jq or a temp file for nontrivial payloads instead of brittle inline quoting.",
...
type PaperclipWakePayload = {
  ...
  resolvedBlockerSummaries: PaperclipWakeBlockerSummary[];
...
function normalizePaperclipWakeBlockerSummary(value: unknown): PaperclipWakeBlockerSummary | null {
  ...
  const summary = asString(blocker.summary, "").trim() || null;
...
export function normalizePaperclipWakePayload(value: unknown): PaperclipWakePayload | null {
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ...
    : [];
  const resolvedBlockerSummaries = Array.isArray(payload.resolvedBlockerSummaries)
    ? payload.resolvedBlockerSummaries
        .map((entry) => normalizePaperclipWakeBlockerSummary(entry))
        .filter((entry): entry is PaperclipWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizePaperclipWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && resolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !taskWatchdog && !normalizePaperclipWakeIssue(payload.issue)) {
    return null;
  }
...
export function renderPaperclipWakePrompt(
  ...
  if (normalized.resolvedBlockerSummaries.length > 0) {
    lines.push("- resolved blockers:");
    for (const blocker of normalized.resolvedBlockerSummaries) {
      const label = blocker.identifier ?? blocker.id ?? "unknown";
      lines.push(
        `  - ${label}${blocker.title ? ` ${blocker.title}` : ""}${blocker.status ? ` (${blocker.status})` : ""}`,
      );
      if (blocker.summary) {
