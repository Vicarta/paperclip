...
async function findCodexSessionFile(input: {
  codexHome: string;
  sessionId: string;
  maxDepth?: number;
}): Promise<string | null> {
  ...
  return await walk(root, maxDepth);
}

async function inspectCodexSessionResumeAccess(input: {
  codexHome: string;
  sessionId: string;
}): Promise<
  | { status: "missing" }
  | { status: "writable"; file: string }
  | { status: "not_writable"; file: string; reason: string }
> {
  const file = await findCodexSessionFile(input);
  if (!file) return { status: "missing" };

  try {
    await fs.access(file, fsConstants.R_OK | fsConstants.W_OK);
    return { status: "writable", file };
  } catch (err) {
    return {
      status: "not_writable",
      file,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    ...
    if (projectId) {
      env.PAPERCLIP_PROJECT_ID = projectId;
    }
    if (issueWorkMode) {
    ...
    let sessionId = canResumeSession && !forceFreshSession ? runtimeSessionId : null;
    ...
    const resumePreflightNotes: string[] = [];
    if (sessionId && !executionTargetIsRemote) {
      const resumeAccess = await inspectCodexSessionResumeAccess({
        codexHome: effectiveCodexHome,
        sessionId,
      });
      if (resumeAccess.status === "not_writable") {
        await onLog(
          "stdout",
          `[paperclip] Skipping saved Codex session "${sessionId}" because its session log is not writable: ${resumeAccess.file} (${resumeAccess.reason}). Starting a fresh session with Paperclip heartbeat context.\n`,
        );
        resumePreflightNotes.push(
          `Skipped saved Codex session ${sessionId} because its session log was not writable; started a fresh session from Paperclip heartbeat context.`,
        );
        sessionId = null;
      }
    }
    ...
    if (resumePreflightNotes.length > 0) {
      commandNotes.push(...resumePreflightNotes);
    }
