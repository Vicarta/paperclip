...
export const modelProfiles: AdapterModelProfileDefinition[] = [
  {
    key: "cheap",
    label: "Cheap",
    description: "Use the standard Codex local model lane for status-only recovery without changing the primary model.",
    adapterConfig: {
      model: "gpt-5.5",
      // Recovery/status-only runs must not fall back to Spark; keep them on the operator-approved Codex model.
