...
function previewResult(payload, reason = "preview") {
    return {
...
function resultAsToolResult(result) {
    return {
        content: result.content,
        data: result.data,
    };
}
function taskIdFromResult(data) {
...
function buildCommentPayload(args, config) {
    if (!isRecord(args))
        throw new Error("Perfex comment input must be an object");
    const taskId = readString(args.task_id);
    if (!taskId)
        throw new Error("task_id is required");
    const body = readString(args.body) ?? readString(args.comment);
    if (!body)
        throw new Error("body or comment is required");
    const normalized = normalizeConfig(config);
    const author = readString(args.author) ?? normalized.projectManagerId;
    if (!author)
        throw new Error("author or configured projectManagerId is required");
    return {
        task_id: taskId,
        author,
        body,
...
function buildGetTaskPayload(args) {
    if (!isRecord(args))
        throw new Error("Perfex task status input must be an object");
    const taskId = readString(args.task_id);
    if (!taskId)
        throw new Error("task_id is required");
...
const plugin = definePlugin({
    async setup(ctx) {
        ...
        ctx.tools.register(TOOL_NAMES.healthcheck, {
            ...
            return resultAsToolResult(await perfexHealthcheck({
                config,
                resolveSecret: (secretRef) => ctx.secrets.resolve(secretRef),
                fetchFn: ctx.http.fetch,
            }));
        });
        ctx.tools.register(TOOL_NAMES.listTools, {
            displayName: "Perfex List MCP Tools",
            description: "List tools exposed by the Perfex MCP server. Read-only.",
            parametersSchema: looseObjectSchema,
        }, async () => {
            const config = await getConfig(ctx);
            return resultAsToolResult(await listPerfexMcpTools({
                config,
                resolveSecret: (secretRef) => ctx.secrets.resolve(secretRef),
                fetchFn: ctx.http.fetch,
            }));
        });
        ctx.tools.register(TOOL_NAMES.previewImplementationTask, {
        ...
        ctx.tools.register(TOOL_NAMES.createImplementationTask, {
            displayName: "Perfex Create Implementation Task",
            description: "Create a Perfex implementation task only when writes are enabled and dry_run is false.",
            parametersSchema: looseObjectSchema,
        }, async (params, runCtx) => {
            const config = await getConfig(ctx);
        ...
        ctx.tools.register(TOOL_NAMES.addTaskComment, {
            displayName: "Perfex Add Task Comment",
            description: "Add a Perfex task comment only when writes are enabled and dry_run is false.",
            parametersSchema: looseObjectSchema,
        }, async (params, runCtx) => {
            const config = await getConfig(ctx);
        ...
        ctx.tools.register(TOOL_NAMES.getTaskStatus, {
            ...
            const config = await getConfig(ctx);
            const normalized = normalizeConfig(config);
            return resultAsToolResult(await callPerfexMcpTool({
        ...
        ctx.tools.register(TOOL_NAMES.getTaskComments, {
        ...
        ctx.tools.register(TOOL_NAMES.syncTaskStatus, {
