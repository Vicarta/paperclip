import { DEFAULT_PERFEX_HEALTH_URL, DEFAULT_PERFEX_MCP_URL, EXPORT_NAMES, PLUGIN_ID, PLUGIN_VERSION, SLOT_IDS, TOOL_NAMES, } from "./constants.js";
...
const manifest = {
    ...
    description: "Server-side adapter for the Perfex CRM MCP endpoint. Agents can preview human implementation tasks, perform read-only status checks, and only create/update Perfex tasks when writes are explicitly enabled.",
    author: "Paperclip",
    ...
    instanceConfigSchema: {
        ...
        properties: {
            perfexMcpTokenSecretRef: {
                ...
                description: "Paperclip secret UUID that stores the Perfex MCP bearer token. Agents never receive the plaintext token.",
                default: "",
            },
            perfexMcpUrl: {
                type: "string",
            ...
            statusCheckIntervalMinutes: {
                ...
                description: "Recommended polling interval for future scheduled status sync. Phase 11 MVP does not auto-schedule sync jobs.",
                default: 60,
            },
            requestTimeoutMs: {
                type: "number",
    ...
    tools: [
        ...
        },
        {
            ...
            description: "Create a Perfex implementation task only when plugin writes are enabled and dry_run is false. Otherwise returns a safe preview.",
            parametersSchema: looseObjectSchema,
        },
        {
            name: TOOL_NAMES.addTaskComment,
            displayName: "Perfex Add Task Comment",
            description: "Add a Perfex task comment only when plugin writes are enabled and dry_run is false. Otherwise returns a safe preview.",
            parametersSchema: looseObjectSchema,
        },
        {
