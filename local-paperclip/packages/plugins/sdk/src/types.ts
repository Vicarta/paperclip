...
export interface ToolRunContext {
  ...
  projectId: string;
  /** UUID of the issue/task the run is handling, when tool execution is issue-scoped. */
  issueId?: string;
  /** UUID of the execution workspace bound to the run, when available. */
  executionWorkspaceId?: string;
  /** Absolute cwd of the execution workspace bound to the run, when available. */
...
export interface PluginCostEventInput extends Omit<CreateCostEvent, "amountMicros" | "currency" | "status"> {
...
export interface PluginCostsClient {
   ...
   *
   * The host enforces company/plugin availability and writes audit activity.
   */
