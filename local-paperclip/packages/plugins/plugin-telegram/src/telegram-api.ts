...
export async function sendDocument(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const blobPart = content.buffer.slice(
        content.byteOffset,
      ...
      form.append(
        "document",
        new Blob([blobPart], { type: attachment.contentType || "application/octet-stream" }),
      ...
      if (!data.ok) {
        if (data.parameters?.retry_after && attempt < 2) {
          const wait = data.parameters.retry_after * 1000;
          ctx.logger.warn("Telegram rate limited while sending document, retrying", {
        ...
        ctx.logger.error("Telegram sendDocument failed", {
          error: data.description,
      ...
      ctx.logger.error("Telegram sendDocument API error", {
        error: String(err),
        attachmentId: attachment.id,
      });
      await ctx.metrics.write(METRIC_NAMES.failed, 1);
