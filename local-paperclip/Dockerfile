...
ARG USER_GID=1000
RUN apt-get update \
  && apt-get install -y --no-install-recommends ca-certificates gosu curl git wget jq ripgrep python3 procps \
...
RUN pnpm --filter @paperclipai/plugin-exa-agent-tools build
