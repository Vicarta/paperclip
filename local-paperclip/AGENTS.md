...
When your task produces a user-inspectable deliverable file, follow the Paperclip skill's "Generated Artifacts and Work Products" workflow before final disposition. In this repo, prefer the self-contained skill helper at `skills/paperclip/scripts/paperclip-upload-artifact.sh` when uploading a local file directly. When the deliverable already exists in the current execution or project workspace, use the typed `POST /api/issues/:id/work-products/register-workspace-artifact` route instead of handcrafting raw work-product JSON. That route validates the workspace-relative path, stores the file as an issue attachment, and creates the canonical attachment-backed `provider=paperclip`, `type=artifact` work product. Raw `POST /api/issues/:id/work-products` with `provider=paperclip` must not be used for workspace-generated deliverables unless an attachment already exists and the raw payload references only that attachment ID. Link the uploaded/registered artifact in the final issue comment, then set status. Do not rely on local filesystem paths as the only access path. Use `metadata.resourceRef.kind: "workspace_file"` only for source/workspace files that are intentionally not deliverables. Treat browse/search as a fallback for recovering workspace files, not the preferred deliverable path. See `doc/AGENT-ARTIFACTS.md` and `skills/paperclip/references/artifacts.md` for details.
