...
Related follow-up idea:

- add a **project-scoped chat tab/window** as a plugin-backed surface inside project detail
- treat it as a project-native command surface, not as a new global chat product
- default all conversation context, outputs, and spawned work to the current project
...
- Improve the workspace file browser ergonomics for long-form text work:
  - default to wrapped lines in the editor;
  - expose a `Wrap On/Off` toggle in the file toolbar;
  - persist the operator's wrap preference locally in the browser
- Improve issue timestamp readability in the properties panel:
  - show `Started`, `Completed`, and `Created` with full date+time;
  - keep `Updated` as relative time but also show a muted absolute timestamp underneath
- Include `ripgrep` in the Paperclip app runtime image so local adapters can reliably use `rg` in shell-based workflows
